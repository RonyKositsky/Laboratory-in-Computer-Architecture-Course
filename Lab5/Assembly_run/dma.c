...
static void assemble_program(char *program_name)
{
	...
	asm_cmd(JLT, 0, 3, 5, 7); 		// 10: if R3 < R5 jump to line 7
	...
	// testing the control hazard by starting copy after branch that not taken
	asm_cmd(ADD, 3, 1, 0, 50); 		// 25: R3 = 50 (src)
	asm_cmd(ADD, 4, 1, 0, 60); 		// 26: R4 = 60 (dst)
	asm_cmd(ADD, 5, 1, 0, 50); 		// 27: R5 = 50 (copy length)
	asm_cmd(JEQ, 0, 0 ,0, 30);		// 28: jump to line 28 in order to cancel the copy command
	asm_cmd(CPY, 4, 3, 5, 0);		// 29: CPY length of R[5] values from R[3] (src) to R[4] (dst)
	// verify that copy not started
	asm_cmd(ASK, 2, 0, 0, 0); 		// 30: R[2] = #copy_transactions_remaining
